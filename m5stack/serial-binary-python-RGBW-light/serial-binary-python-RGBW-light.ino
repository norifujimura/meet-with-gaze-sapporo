...
void setup() {
  M5.begin();
  Serial.begin(500000);// up tp 500000
  M5.Lcd.fillScreen(BLACK);
    M5.Lcd.setTextSize(2);
  M5.Lcd.println("OK RGBW 50000");
}
...
void serialEvent() {
  ...
  if (Serial.available() > 0) {
    char c = char(255);
    rLen = Serial.readBytesUntil(c, buf, (len*4+1));
    if(rLen==(len*4)){
      showValue();
      showLED();
    }
  ...
  */
  ...
  for(int i = 0;i<len;i++){
    ...
    */
    ...
    M5.Lcd.fillRect(i, 120,1,60, rgbColor);
    if(w>0){
      M5.Lcd.fillRect(i, 180,1,60, wColor);
    }
